...
func main() {
	...
	r.GET("/movies", func(ctx *gin.Context) {
		res, err := client.GetMovies(ctx, &moviepb.ReadMoviesRequest{})
		if err != nil {
	...
	r.GET("/movies/:id", func(ctx *gin.Context) {
		id := ctx.Param("id")
		res, err := client.GetMovie(ctx, &moviepb.ReadMovieRequest{Id: id})
		if err != nil {
	...
	r.POST("/movies", func(ctx *gin.Context) {
		...
		res, err := client.CreateMovie(ctx, &moviepb.CreateMovieRequest{
			Movie: data,
		})
		if err != nil {
			ctx.JSON(http.StatusBadRequest, gin.H{
	...
	r.PUT("/movies/:id", func(ctx *gin.Context) {
		...
		res, err := client.UpdateMovie(ctx, &moviepb.UpdateMovieRequest{
			Movie: &moviepb.Movie{
				Id:    movie.ID,
				Title: movie.Title,
				Genre: movie.Genre,
	...
	r.DELETE("/movies/:id", func(ctx *gin.Context) {
		id := ctx.Param("id")
		res, err := client.DeleteMovie(ctx, &moviepb.DeleteMovieRequest{Id: id})
		if err != nil {
			ctx.JSON(http.StatusBadRequest, gin.H{
				"error": err.Error(),
			})
